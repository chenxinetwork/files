...
>>>>>>> bb00a41f14ec232f9fd62519187bc465f3bb5c07
	...
	echo '' > aria2.session
	sed -i 's/^rpc-secret=DOUBIChenxi/rpc-secret='$(date +%s%N | md5sum | head -c 20)'/g' ${aria2_conf}
}
Service_aria2(){
	if [[ ${release} = "centos" ]]; then
<<<<<<< HEAD
		if ! wget --no-check-certificate https://raw.githubusercontent.com/chenxinetwork/files/master/service/aria2_centos -O /etc/init.d/aria2; then
=======
		if ! wget --no-check-certificate https://gitee.com/chenxinetwork/files/raw/master/service/aria2_centos -O /etc/init.d/aria2; then
>>>>>>> bb00a41f14ec232f9fd62519187bc465f3bb5c07
...
<<<<<<< HEAD
		if ! wget --no-check-certificate https://raw.githubusercontent.com/chenxinetwork/files/master/service/aria2_debian -O /etc/init.d/aria2; then
=======
		if ! wget --no-check-certificate https://gitee.com/chenxinetwork/files/raw/master/service/aria2_debian -O /etc/init.d/aria2; then
>>>>>>> bb00a41f14ec232f9fd62519187bc465f3bb5c07
			echo -e "${Error} Aria2服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/aria2
...
Set_aria2_vim_conf(){
	...
	echo -e "${Tip} 手动修改配置文件须知（nano 文本编辑器详细使用教程：linux-jc13/）：
...
Update_Shell(){
<<<<<<< HEAD
	sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://raw.githubusercontent.com/chenxinetwork/files/master/aria2.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
=======
	sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://gitee.com/chenxinetwork/files/raw/master/aria2.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
>>>>>>> bb00a41f14ec232f9fd62519187bc465f3bb5c07
