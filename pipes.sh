...
Service_pipes(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://raw.githubusercontent.com/chenxinetwok/files/master/service/pipes_centos -O /etc/init.d/pipes; then
		...
		if ! wget --no-check-certificate https://raw.githubusercontent.com/chenxinetwok/files/master/service/pipes_debian -O /etc/init.d/pipes; then
			echo -e "${Error} ShadowsocksR服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/pipes
